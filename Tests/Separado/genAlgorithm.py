...
def animate(gens, interval : int = 500):
        fig = plt.figure(1)
        ax = plt.axes(xlim=[-2, 2], ylim=[-2, 2])

        x = [ind.get_x() for ind in gens[0]]
        y = [ind.get_y() for ind in gens[0]]
        scatter = ax.scatter(x, y)

        def update(i):
            xy = [[ind.get_x(), ind.get_y()] for ind in gens[i]]
